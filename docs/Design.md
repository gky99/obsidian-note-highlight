...
**The reading-mode flash rides on the highlight span, so the in-place reconcile must not clobber it (fixed 2026-06-29).** Because the reading flash is *the same span* that carries the highlight (no separate decoration layer like CM6), it shares the span's `class`/`style` with the §14.3.1 in-place reconcile (`syncReadingHighlights`). A jump fires that reconcile **within the flash window** — `jumpToAnnotation` calls `leaf.openFile`, whose `file-open`/`active-leaf-change` → `store.load` → `repaint` → `scheduleHeal` runs an immediate **and a deferred** `syncReadingHighlights` pass. The recolor step (`colorHighlightSpan`) originally reassigned `span.className` wholesale, which **stripped the transient `mrg-flash` class** the jump had just added — so the highlight stayed but the glow vanished, *reading-mode only* (Live Preview's flash is a separate CM6 decoration field, untouched by the reconcile). Fix: recolor by swapping **only** the `mrg-color-*` token class (via `classList`), preserving `mrg-highlight` and any transient class. The reading-flash helper is unit-tested (happy-dom), the recolor-preserves-`mrg-flash` invariant is unit-tested (`reading.test.ts`), and the in-vault `jump-flash.e2e.ts` now asserts the flash **survives a forced heal pass** (teeth-checked: clobber `className` → it fails).
