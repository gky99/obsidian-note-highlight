...
- **The DOM match is whitespace- and block-boundary-insensitive (lesson learned, 2026-06-27).** The needle is whitespace-collapsed by `projectQuoteToText`, but the rendered text nodes are *not* — so a raw `concat.indexOf(needle)` fails wherever the two diverge in whitespace. Two cases bit us: (1) a *soft line break* (one source newline inside a paragraph) survives in the DOM as a literal `"\n"` between text nodes, so `"… notepad and …"` (needle, space) wasn't found in `"… notepad\nand …"` (DOM); and (2) a **block boundary** (separate paragraphs / list items) contributes *no character at all* between text nodes, so a cross-paragraph quote concatenated to `"…notepadand a…"` with the space missing entirely. `highlightFirstMatch` therefore matches against a whitespace-**collapsed** projection of the concatenation (with an index map back to raw offsets for the wrap), and inserts a **synthetic space at block boundaries** (`nearestBlock`: the nearest block-ancestor changed). The per-section post-processor passes one block as `root`, so the synthetic-space logic is a no-op there; it only matters when matching across the whole preview (next bullet). Live Preview was always fine — CM6 uses source offsets, never DOM text. Guarded by `reading.test.ts` ("soft line break", self-heal cases) and `mid-bold.e2e.ts`.
- **Self-heal: paint reliably even when the post-processor doesn't run (lesson learned, 2026-06-27).** A highlight could end up **rendered but unpainted** — `anchored`, its text in the DOM, yet zero `.mrg-highlight` spans — *intermittently*, when toggling Live Preview ↔ Reading on a long note. The render lifecycle is the cause, not matching: `previewMode.rerender(true)` clears a section and Obsidian re-attaches **cached** sections without reliably re-running post-processors, so the per-section painter sometimes never paints that section. (`getResolved` is atomic — the store data is always present; the gap is purely *when the painter runs*.) The fix is a self-heal: after render-affecting events (`repaint`, mode flip) the plugin runs an **idempotent** whole-preview painter (`reading.ts#paintMissingHighlights` via `main.ts#healReadingViews`/`scheduleHeal`, immediate + one deferred pass) that paints only anchored highlights with **no** span yet, matching the whole projected quote across the *entire* `.markdown-preview-view` (so it needs no `getSectionInfo` slice — hence the block-boundary handling above). **Lesson:** a render-time post-processor is necessary but not *sufficient* for reliable painting — Obsidian's section caching makes it racy, so a content-based, idempotent reconciliation pass over the rendered DOM is the durable backstop. Verified by `reading-flaky.e2e.ts`: strip the painted spans to reproduce the exact symptom, run the heal, the highlight repaints. The intermittent race itself was **not** reproducible headless (clean vault + programmatic mode-switching never triggered the cache reuse) — so the test targets the observable *symptom* (rendered-but-unpainted → repainted), which is what the heal deterministically fixes.
...
**The post-jump flash (both modes).** A jump lands the reader somewhere new, so the target highlight gets a transient **`mrg-flash`** animation — a soft accent glow *inside* the highlight that fades in, holds, then fades out (one flash, ≈1.67 s) — to draw the eye to *where* it landed. It's an **inset** box-shadow fill (via `color-mix` over `--interactive-accent`), so it layers on top of any highlight color rather than replacing it or drawing a boxy outer ring. Two delivery mechanisms, one CSS class, mirroring how highlights themselves are painted (§7.2): in **edit/Live-Preview** it's a CM6 decoration mark over the range (`src/editor/flash.ts`, mapped across edits, cleared after the animation); in **reading mode** — which has no CodeMirror — it's the same class toggled directly on the *painted* `.mrg-highlight[data-anno-id]` spans (`src/reading/flash.ts#flashReadingHighlights`, the same elements scroll-sync reads). Reading mode paints a section as it scrolls into view, so `navigation.flashReadingMode` polls a few frames until the spans exist before flashing. Earlier this was editor-only (reading mode had no flash, so a reading-mode jump looked like nothing happened).

**The inset shadow must share an element with the highlight background (the two-mode layering trap, fixed 2026-06-29).** An `inset` box-shadow paints over its *own* element's background but **under** a child element's background (CSS paints a child after the parent's background + inset shadow). The two modes have different DOM shapes: reading mode paints **one** span carrying *both* classes (`.mrg-highlight.mrg-flash`), so the accent sits over the highlight tint — correct. But Live Preview is **two nested CM6 decoration spans** with `.mrg-flash` on the **outer** one (`.mrg-flash > .mrg-highlight` — the flash mark wraps the highlight mark), so the outer inset shadow painted *under* the inner highlight's (translucent) background and read as "no flash" — the symptom was a working reading-mode flash with a silently invisible Live-Preview one. Because the structure is fixed by CM6 decoration precedence and never changed, the Live-Preview flash was rendering under the tint since it shipped; it only "broke" perceptually against a colored highlight. Fix: run the animation on the inner highlight span too (`.mrg-flash, .mrg-flash .mrg-highlight` in `styles.css`); its own inset shadow then paints over its own background, matching reading mode. The descendant selector can't match the single dual-class reading-mode span, so there's no double-flash. The reading-flash helper is unit-tested (happy-dom); both modes are verified in-vault — `jump-flash.e2e.ts` (reading) and `jump-flash-live-preview.e2e.ts` (Live Preview, asserting the *visible* highlight surface runs the `mrg-flash` animation) — both teeth-checked.
...
**Boundary delimiters travel with their span (fixed 2026-06-26).** The match is bounded by the
first/last *rendered* (non-marker) characters, so an emphasis delimiter that opens or closes
*exactly* at a highlight boundary fell just outside `[from,to)` while its partner stayed inside —
e.g. a mark beginning at `**bold**` kept the interior closing `**` but dropped the opening one,
storing the broken, unbalanced quote `bold** …`. (Reported as "if the bold is at the start of the
highlight, the start of the bold isn't included.") `locateMark` now grows the range over the
emphasis runs hugging each end and keeps the boundary combination whose quote is *balanced* (an
even number of runs for every delimiter char it would pull in) — so it adds the opening `**`/`*`/
`` ` ``/`~~` (and the symmetric trailing case) but *won't* grab the opener of a following span
(`word*italic*` highlighted as "word" stays `word`, not `word*`). Balance is judged only against
the delimiter chars at the boundary, so an interior `snake_case`/URL underscore never blocks it;
when nothing balances the raw range is returned unchanged. This is purely about keeping the stored
*quote* well-formed Markdown — re-anchoring is unaffected either way, since the resolver strips all
markers before matching. Guarded by `locate.test.ts` (leading/trailing/exact-wrap/multi-span +
the "following span" negative case).

**Shared with manual select-and-mark (2026-06-27).** The same balancing belongs on *every*
creation path, not just import: in Live Preview the `**` markers are concealed, so a selection that
begins at the bold *content* yields editor offsets starting after the opening `**` — the identical
`bold** …` defect. The balancer was extracted to a pure `src/text/emphasis.ts#balanceEmphasisRange`
(unit-tested) and is now applied by `locateMark` (import) **and** `store.createHighlight` /
`createHighlights` (manual toolbar, command, import apply) so all paths store the same balanced
quote. `base` (`bodyStart`) bounds the leading extension so it can't pull into the frontmatter.
Guarded in-vault by `select-mark.e2e.ts` (teeth-checked: neutralize → the leading `**` is dropped).
