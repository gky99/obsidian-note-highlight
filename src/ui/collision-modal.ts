 ...
 *    (`Note-1.annotations.md`) — keeps the two notes independent.
 *  - **Continue (share file)**: store this note's annotations in the existing
 *    sidecar; both notes then read it best-effort (the off-note's may orphan).
  ...
  onOpen(): void {
    ...
    contentEl.createEl('p', {
      text:
        ...
        `which shares this note’s file name. Saving here would mix their annotations.`,
    });
    contentEl.createEl('p', { text: 'How should this note’s annotations be saved?' });
    ...
    new Setting(contentEl)
      ...
      .addButton((b) =>
        b.setButtonText('Continue (share file)').onClick(() => this.pick('continue')),
      )
